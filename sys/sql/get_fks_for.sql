...
begin
 ...
 for fks in (select fk.owner fk_owner, fk.table_name fk_table_name,fk.constraint_name fk_constraint_name, 
                    pk.owner pk_owner, pk.constraint_name pk_constraint_name
...
loop
 dbms_output.put_line(fks.fk_owner||'.'||fks.fk_table_NAME);
 dbms_output.put_line('.     '||fks.fk_constraint_name||' references '||fks.pk_constraint_name);
 fk_cols:='(';
 pk_cols:='(';
...
order by fkc.position)
 ...
 dbms_output.put_line(fk_cols||' -- '||pk_cols);
end loop;
end;
/